...
public class UserController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(command.equals("/registForm.user")) {
		...
		}else if(command.equals("/home.user")) { // index(로그인 화면)이동
			...
			if(ldto==null||ldto.getId()==null) {
	            ...
	            if(ldto.getRole().toUpperCase().equals("ADMIN")) {
	        	 ...
	        	 if(isS) {
	        		 // /path : 상위root에서, ../path : 현재위치에서 한 단계 위, ./path : 현재경로에서, path : 왼쪽과 동일
	          ...
	          }else if(command.equals("/getUserList.user")) { // 회원정보수정
