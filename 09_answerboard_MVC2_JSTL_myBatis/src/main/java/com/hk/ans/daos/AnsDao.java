...
public class AnsDao extends SqlMapConfig{
	...
	//1.글목록 조회하기
	/*
	 * public List<AnsDto> getAllList(){ List<AnsDto> list=new ArrayList<>();
	 * SqlSession sqlSession=null;//쿼리를 실행시켜주는 객체 try { //sqlSessionFactory객체의
	 * openSession()를 통해 //sqlSession객체를 구한다 (true:autocommit설정)
	 * sqlSession=getSqlSessionFactory().openSession(true);
	 * 
	 * //sqlSession객체에 쿼리 실행 메서드를 통해 쿼리실행 // 쿼리실행메서드(쿼리이름,파라미터)
	 * list=sqlSession.selectList(namespace+"boardList"); } catch (Exception e) {
